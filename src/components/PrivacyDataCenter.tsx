          ...
          <div className="p-4 rounded-2xl bg-gray-50 border border-gray-200 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                Health Metrics Processing Consent
              </span>
              <span
              ...
              >
                {profile.healthDataConsent && !profile.isConsentWithdrawn ? 'Active & Granted' : 'Withdrawn / Paused'}
              </span>
            </div>
            <p className="text-xs text-gray-600 leading-relaxed">
              Authorizes Fitkode to process body metrics, dietary logs, and questionnaire answers solely for custom fitness programming.
            </p>
            {profile.healthDataConsentGivenAt && (
          ...
          <div className="p-4 rounded-2xl bg-gray-50 border border-gray-200 space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-gray-700 uppercase tracking-wider flex items-center gap-1.5">
                ...
                Coaching &amp; Plan Notifications
              </span>
              <span
                className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${
                  profile.notificationsConsent
                    ? 'bg-emerald-100 text-emerald-800 border border-emerald-300'
                    : 'bg-gray-100 text-gray-700 border border-gray-300'
                }`}
              >
                {profile.notificationsConsent ? 'Opted In' : 'Opted Out'}
              </span>
            </div>
            <p className="text-xs text-gray-600 leading-relaxed">
              Controls workout reminders, weekly check-in alerts, and nutrition guidance messages via email and WhatsApp.
            </p>
            {profile.notificationsConsentGivenAt && (
              <div className="text-[11px] text-gray-500 flex items-center gap-1">
                <Clock className="w-3 h-3 text-gray-400" />
                <span>Updated: {new Date(profile.notificationsConsentGivenAt).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} IST</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-3xl border border-brand-light-green p-6 shadow-xs flex flex-col justify-between space-y-4">
          <div className="space-y-3">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              Under DPDPA Section 6, you can revoke consent to process your health indicators at any time. Coaching program adjustments will be paused until consent is re-granted.
            </p>
          </div>
